...
from tests.common.gu_utils import (apply_patch, generate_tmpfile, delete_tmpfile,
                                    ...
                                    expect_op_success, format_json_patch_for_multiasic)
...
def test_perf_acl_table_add(perf_ctx):
    ...
    Exercises ADD move generation and CreateOnlyMoveExtender validation.
...
def test_perf_port_mtu_replace(perf_ctx):
    """
    Replace MTU on multiple ports. Tests REPLACE on existing scalar
    config entries without leaf-list decomposition.
    """
    duthost = perf_ctx['duthost']
    up_ports = perf_ctx['up_ports']
    loaddata_time = perf_ctx['loaddata_time']
